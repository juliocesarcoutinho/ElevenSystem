...
const DeleteUser: React.FC<DeletarUsuarioProps> = ({
  usuario,
  isOpen,
  onHide,
  onUpdate,
  ...
  const [usuarioSelecionado, setUsuarioSelecionado] =
    useState<Demo.Usuario | null>(null);
  const [deleteUsuarioDialog, setDeleteUsuarioDialog] =
  ...
  const deleteUsuario = async (usuario: Demo.Usuario) => {
    if (usuario.id != null) {
      try {
        ...
        const errorMessage =
          error.message === "Operador não autorizado para excluir usuário"
            ? "Operador não autorizado para excluir usuário"
